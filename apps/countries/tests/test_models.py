...
from apps.utils import in_dollar, world_population, global_gdp
...
class TestCountryProperties:
    ...
    cf = CountryFactory.build(internal_debt=internal_debt, external_debt=external_debt, population=population, gdp=gdp)
    ...
    def test_external_debt_per_citizen(self):
        ...
        assert(self.cf.external_debt_per_citizen == in_dollar.format(ext_debt_per_citizen))
    
    def test_total_debt_per_citizen(self):
        total_debt_per_citizen = self.total_debt_as_int / self.population
        assert(self.cf.total_debt_per_citizen == in_dollar.format(total_debt_per_citizen))
    ...
    def test_population_percentage_of_the_world(self):
        ...
        assert(self.cf.population_percentage_of_the_world == f"{str(country_vs_world_population)}%")
